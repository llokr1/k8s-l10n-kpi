...
type PrSize = (typeof prSizeOrder)[number];
...
export default function Dashboard() {
  ...
  const filteredApprovers = useMemo(() => (data.approvers || []).map((approver) => ({
    ...
    approvedPullRequests: approver.approvedPullRequests.filter((item) => item.approvalMethods?.includes("approve_command") && inRange(item.approvedAt || item.updatedAt, from, to)),
  ...
  return (
    <main>
      ...
      {view === "members" && <section className="contentSection">
        <div className="sectionHead"><h2>멤버별 기여</h2><p>카드에서 PR 사이즈별 생성 횟수를 비교하고, 선택하면 상세 활동을 볼 수 있습니다.</p></div>
        ...
        <div className="memberGrid approverGrid">{filteredApprovers.map((approver) => {
          ...
          return <button className="memberCard approverCard" key={approver.githubId} onClick={() => setApproverId(approver.githubId)}><div className="memberTop"><img src={`https://github.com/${approver.githubId}.png?size=96`} alt="" /><span><b>{approver.name}<i className="roleBadge">Approver</i></b><small>@{approver.githubId}{approver.member ? " · 아카데미 멤버" : ""}</small></span><strong>{activityPulls}</strong></div><div className="miniStats approverStats"><span><b>{approver.reviewedPullRequests.length}</b>리뷰 PR</span><span><b>{approver.approvedPullRequests.length}</b>승인 PR</span></div></button>;
        })}</div>
      </section>}
