    ...
    {tab === "prs" ? <>
      ...
      <div className="prTableWrap" tabIndex={0} role="region" aria-label="PR 관리 표 · 가로 스크롤 가능"><table className="prGrid"><thead><tr><th>PR</th><th>작성자</th><th>Issue</th><th>이슈 생성</th><th>PR 생성</th><th>size</th><th>Status</th><th className="prManualColumn">Reviewer</th><th>Review 완료</th><th>LGTM</th><th>Approver</th><th>관리 상태</th><th>기한</th><th>비고</th></tr></thead><tbody>
        {rows.map((pr) => {
          ...
          const rowClassName = [state === "excluded" && "prExcluded", pr.state === "merged" && "prMerged"].filter(Boolean).join(" ");
          return <tr key={pr.number} className={rowClassName}>
            ...
            <td className={pr.state === "merged" ? "prMergedStatus" : undefined}>{pr.state === "merged" ? <strong className="prMergedStamp">Merged</strong> : <><span className={"prState " + pr.state}>{pr.draft ? "Draft" : pr.state === "closed" ? "Closed" : pr.state === "open" ? "Open" : "확인 중"}</span>{pr.labels.includes("do-not-merge/hold") && <small>hold</small>}</>}</td>
