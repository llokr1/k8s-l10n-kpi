...
export function workload(prs, assignments, login) {
  const eligible = prs.filter(pr => workState(pr, assignments[pr.number]) !== "excluded");
  const assigned = eligible.filter((pr) => assignedReviewers(pr, assignments[pr.number]).includes(loginKey(login)));
  ...
  return { assigned: assigned.length, completed: assigned.filter(done).length, pending: pending.length, totalWeight: assigned.reduce((sum, pr) => sum + weight(pr), 0), pendingWeight: pending.reduce((sum, pr) => sum + weight(pr), 0), unknownSize: assigned.filter((pr) => !weight(pr)).length,
    sizes: Object.fromEntries(Object.keys(SIZE_WEIGHTS).map(size => [size, assigned.filter(pr => pr.size?.replace("size/", "") === size).length])),
    voluntary: eligible.filter(pr => !assigned.includes(pr) && done(pr)).length,
